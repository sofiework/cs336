...
class FFN(nn.Module):
    def __init__(self, d_model: int, d_ff: int=None, device=None, dtype=None):
        ...
        self.w1 = Linear(d_model, self.d_ff, device=device, dtype=dtype) # up proj
        self.w2 = Linear(self.d_ff, d_model, device=device, dtype=dtype) # down proj
        self.w3 = Linear(d_model, self.d_ff, device=device, dtype=dtype) # same as up proj
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model: int, num_head: int, d_ff: int, theta: float, max_seq_len: int,
        ...
        # modules
        self.norm1 = RMSNorm(self.d_model, device=device, dtype=dtype) # each has own learnable g
        self.norm2 = RMSNorm(self.d_model, device=device, dtype=dtype)
        self.causal_runner = Causal_multihead_attn(self.d_model, self.num_head, self.theta, self.max_seq_len, device=device, dtype=dtype)
        self.ffn = FFN(self.d_model, self.d_ff, device=device, dtype=dtype)
...
class Transformer_LM(nn.Module):
    def __init__(self, d_model: int, num_head: int, d_ff: int, 
        ...
        self.norm_final = RMSNorm(self.d_model, device=device, dtype=dtype) 
        self.lm_head = Linear(d_model, vocab_size, device=device, dtype=dtype)
        # num_layers Transformer blocks
        self.layers = nn.ModuleList([Transformer(d_model, num_head, d_ff, theta, max_seq_len, device=device, dtype=dtype)
